...
/**
 * Represents an TA  that throw questions or information.
 */
public class TA extends KnowledgeKeeper {
    ...
    /**
     * Creates an TA enemy with a name, position, icon, and image path.
     *
     * @param name       the name of  ta
     * @param x          the x-coordinate of  ta
     * @param y          the y-coordinate of  ta
     * @param icon       the icon of  ta
     * @param imagePath  path to  ta image file
     */
    public TA(String name, int x, int y, BufferedImage icon, String imagePath) {
        ...
        if (icon != null) {
            this.imageIcon = new ImageIcon(icon.getScaledInstance(80, 80, Image.SCALE_SMOOTH));
        }
    }

    /**
     * Makes the TA shoot a ShotBox
     *
    ...
    public ShotBox shoot(int level) {
        ...
        shot.setDamageOrPoints(isQuestion ? -7 : 15);
        shot.setSpeed(getShotSpeed()); 
        return shot;
    }

    /**
     * Draw  TA on  screen using  image 
     *
